...
- **Auth at the platform edge.** The app ships no bespoke auth; the packet write routes are single-operator, localhost-only, with a same-origin guard, and for real data you put sign-in in front of it (e.g. Azure "Easy Auth" / Entra), so there's no auth code to get wrong.
